...
export class TrainingPage {
  ...
  async getTrainingFaces() {
    ...
    if (!imagesAlreadyStored) {
      ...
      await this.http.put("https://psych-capstone.herokuapp.com/getTrainingFaces/", {level: this.userLevel}, httpOptions).subscribe((res) => {
        for (let i = 0; i < 8; i++) {
  ...
  async getWhosNewFaces() {
    ...
    await this.http.put("https://psych-capstone.herokuapp.com/getWhosNewFaces/", {level: this.userLevel}, httpOptions).subscribe((res) => {
      for (let i = 0; i < 8; i++) {
        facePaths.push(`data:image/png;base64,${res['images'][i]}`)
      }
    });
  ...
  async getDailyAssessmentFaces() {
    ...
    if (!imagesAlreadyStored) {
      ...
      await this.http.put("https://psych-capstone.herokuapp.com/getDailyAssessmentFaces/", {}, httpOptions).subscribe((res) => {
        for (let i = 0; i < 8; i++) {
  ...
  async getPrePostAssessmentFaces() { // Too many to store in local/session storage
    ...
    await this.http.put("https://psych-capstone.herokuapp.com/getPrePostAssessmentFaces/", {}, httpOptions).subscribe((res) => {
      for (let i = 0; i < 30; i++) {
        facePaths.push(`data:image/jpg;base64,${res['images'][i]}`)
      }
    });
